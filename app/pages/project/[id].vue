...
const project = ref<ProjectGetRes | null>(null);

const loadProject = async () => {
  loading.value = true;
  try {
    project.value = await http.get<ProjectGetRes>(`/project/${projectId}`);
  } finally {
    loading.value = false;
  }
};
...
const treeItems = computed<TreeItem[]>(() => {
  ...
  return project.value.endpointGroups.map(convertToTreeItem);
});
...
const selectApi = (ep: ProjectGetResEndpoint | null) => {
  if (ep) {
    apiDetail.value = ep;
  } else {
    apiDetail.value = undefined;
  }
  console.log(apiDetail.value, project.value);
};
</script>
      ...
      <template #api>
        <div class="flex gap-6">
                ...
                <div class="space-y-6">
                  ...
                  <div v-if="apiDetail.response">
                    ...
                    <div class="space-y-3">
                      <div class="flex items-center space-x-2">
                        <UBadge
                          :color="apiDetail.response.status === 200 ? 'success' : 'error'"
                          variant="solid"
                          size="sm"
                        >
                    ...
                    <div class="space-y-4">
                      ...
                      <div class="mb-4">
                        ...
                        <div class="bg-muted p-3 rounded-lg">
                          <pre
                            class="text-sm"
                          ><code>{{ JSON.stringify(apiDetail.response.headers, null, 2) }}</code></pre>
                        </div>
                      </div>
          ...
          <div v-else class="min-w-0 flex-[2_1_66.67%]">
            <ApiListTable :project="project" :loading="loading" @select="selectApi" @reload="loadProject" />
          </div>
        </div>
      </template>
      <template #members>
