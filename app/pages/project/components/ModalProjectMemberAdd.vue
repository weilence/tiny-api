...
const onSubmit = async (e: FormSubmitEvent<Schema>) => {
  await http.post(`/api/project/${props.projectId}/members`, e.data);
  emit('close', true);
};
</script>
...
<template>
  <UModal title="添加成员">
    <template #body>
      <UForm id="add-project-member" :schema="schema" :state="form" @submit="onSubmit">
    ...
    <template #footer>
      <div class="flex gap-2 justify-end w-full">
        <UButton color="primary" type="submit" form="add-project-member">确定</UButton>
