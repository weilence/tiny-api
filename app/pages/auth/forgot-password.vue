<template>
  <div class="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-900 py-12 px-4 sm:px-6 lg:px-8">
    <div class="max-w-md w-full space-y-8">
      <div>
        <h2 class="mt-6 text-center text-3xl font-extrabold text-gray-900 dark:text-white">重置密码</h2>
      ...
      <!-- 成功提示 -->
      <UCard v-if="emailSent" class="p-6 border-green-200 bg-green-50 dark:bg-green-900/20">
        <div class="flex items-center space-x-2">
          <UIcon name="i-heroicons-check-circle" class="w-5 h-5 text-green-600" />
          <div>
            <h3 class="text-sm font-medium text-green-800 dark:text-green-200">邮件已发送</h3>
            <p class="text-sm text-green-600 dark:text-green-300 mt-1">请检查您的邮箱并点击重置链接来设置新密码</p>
          </div>
        </div>
      </UCard>
    </div>
